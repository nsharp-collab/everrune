...
bodyGyro.Changed:connect(function()
	warn("gyro changed to",bodyGyro.CFrame.p)
end)
bodyPos.Changed:connect(function()
	warn("bodyPos changed to",bodyPos.Position)
end)
...
bodyPos.Position = Vector3.new(destination.X,10,destination.Z)
print("setting bodyPos to",bodyPos.Position)

bodyGyro.CFrame = CFrame.new(
	Vector3.new(root.Position.X,root.Position.Y,root.Position.Y),
	Vector3.new(destination.X,root.Position.Y,destination.Z)
)
print("setting cframe of bodygyro to",bodyGyro.CFrame.p)

elseif stance == "attack" then
	print("ATTACKING")
bodyPos.Position = destination
bodyGyro.CFrame = CFrame.new(root.Position,Vector3.new(destination.X,-11,destination.Z))
...
destination = (
	CFrame.new(
	root.CFrame.p,
	Vector3.new(v.Parent.PrimaryPart.CFrame.p.x, -6,v.Parent.PrimaryPart.CFrame.p.z)
	)
	*CFrame.new(0,0,-50)).p
break
...
scanSurroundings()
					...
					<string name="ScriptGuid">{B177AEF5-B6AB-4856-B462-D33827EECAC5}